  ...
  <div class="all-notice">
    ...
    <p class="tip" style="text-align: center; line-height: 30px">
      ...
      <span v-if="isAllLoad">
        ...
        <span v-else> <icon name="null" scale="40" width="40"></icon>没有相关公告 </span>
      </span>
    </p>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async setAllNotice() {
      //查询时init一些数据
      if (this.title || this.categoryId) {
        if (this.title.length > 20) {
          this.$message.error('输入的标题太长啦~~~')
        }
        this.currPage = 1
        this.isAllLoad = false
        this.allNotice = []
      ...
      this.isLoading = true
      const data = await searchAllNotice({
    ...
    async setCategoryData() {
      ...
      if (res.code === 200) {
        this.categoryData = [{id:-1,newsCategoryName:'所有分类'}].concat(res.data.records)
