      ...
      <el-main :style="{'height':size.maxH-120+'px'}">
        ...
        <div v-else class="content" ref="main-content" :style="{width}">
          ...
          <siae-paing
            ...
            :pageSize="resource.size"
            :currentPage="resource.current"
            :btnCount="3"
            @pageChange="setResource"
          ></siae-paing>
        </div>
      </el-main>
    </el-container>
...
import { getResourceSort, getAllResource, download } from '@/api/resource'
export default {
  components: { SiaePaing, TagPage, SiaeLoading, ResourceList },
  data() {
    return {
      resource: {},
      sort: [],
...
<style>
.siae-resource .el-container {
  position: fixed;
  width: 100%;
}
.siae-resource .el-container .el-header {
  color: white;
  display: flex;
  align-items: center;
}
.siae-resource .el-container .el-main {
  background-color: white;
}
.siae-resource .el-container .content {
  margin: auto;
  overflow: auto;
}
.siae-resource .search {
...
.siae-resource .search input {
  background: none;
  border: none;
  color: #999999;
}
.siae-resource .search-data_box {
  border-bottom: 1.2px solid #e0e0e0;
  text-align: center;
  padding: 10px 0;
}
.siae-resource .search-text {
  padding: 3px 10px;
}
</style>