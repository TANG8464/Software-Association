    ...
    <el-container>
      <el-header>
        <div class="content" :style="{ width }">
          <span style="font-size: 25px" @click="upOneLevel()">资源下载</span>
...
import { getAllResource, download } from '@/api/resource'
import { getResourceCategory } from '@/api/resource/category'
export default {
  components: {
    SiaePaing,
    TagPage,
  ...
  methods: {
    ...
    async setResource(current) {
      ...
      const data = await getAllResource(current, 8, this.activeSortId, this.searchData)
      if (data.code === 200) {
    ...
    async setSort() {
      const data = await getResourceCategory({ limit: 9999, flag: true })
      if (data.code === 200) {
