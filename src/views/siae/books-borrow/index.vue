<template>
  <div class="siae-books">
    <el-container>
      ...
      <el-main :style="{'height':size.maxH-120+'px'}">
        ...
        <div v-else class="content" ref="main-content" :style="{width}">
          ...
          <siae-paing
            ...
            @pageChange="setBooks"
          ></siae-paing>
          <my-books :isOpen.sync="isOpenMyBooks" :myBooks="myBooks" @backClick="backClick"></my-books>
          <span class="bookcase">
...
import { getAllBooks, getAllSort, bookBorrow, bookReturn, borrowSearch } from '@/api/books'
import { getActiveUserInfo } from '@/api/active-user'
export default {
  components: { BookList, SiaePaing, TagPage, MyBooks, SiaeLoading },
  data() {
    return {
      books: {},
      sort: [],
  ...
  methods: {
    ...
    async setBooks(current) {
      ...
      const data = await getAllBooks('', this.activeSortId, this.searchData, current, -1, 8)
      if (data.code === 200) {
        this.books = data.data
      } else this.books = { records: [] }
      setTimeout(() => {
    ...
    async setMyBooks() {
      const returnedData = await borrowSearch(null, null, null, 1, this.myId, null, 1000, null)
      const notReturnData = await borrowSearch(null, null, null, 2, this.myId, null, 1000, null)
      const returnedRecords = returnedData.data.records
      const notReturnRecords = notReturnData.data.records
      ...
      if (notReturnData.code === 200) {
        ...
        notReturnRecords.forEach((item) => {
          notReturn.push(Object.assign(item.book, { isNotReturn: true, notReturnId: item.id }))
        })
    ...
    async backClick(bookID) {
      let data, msg
      if (bookID) {
        data = await bookBorrow(bookID, this.myId)
        msg = '借书成功，看完后请尽快归还'
        if (data.code === 200) {
