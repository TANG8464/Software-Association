  ...
  <div class="message">
    <div class="header">
      ...
      <el-input
        ...
        placeholder="请输入消息标题"
        v-model="title"
        @keypress.enter.native="setAllMessage(1)"
      ></el-input>
    </div>
    <el-table
      class="remove"
    ...
    >
      ...
      <el-table-column type="expand">
        <template slot-scope="props">
          ...
          <p style="text-align: right; font-size: 13px">
            {{ props.row.createTime | dateFormatter }}
          </p>
        </template>
      </el-table-column>
        ...
        <template slot-scope="scope">
          <span v-if="scope.row.member">
            <img
              ...
              :src="scope.row.member.avatarUrl"
              width="40"
              height="40"
            />
          </span>
      ...
      <el-table-column
        ...
        align="center"
      ></el-table-column>
      <el-table-column label="阅读人数" align="right" width="100">
        <template slot-scope="scope">
          <span>{{ scope.row.readCount }}/{{ scope.row.readCount + scope.row.notReadCount }}</span>
        </template>
      </el-table-column>
      <el-table-column align="right" width="150">
        <template slot="header">
          ...
          <div v-if="isShowDels">
            <el-button type="danger" size="mini" round plain @click="deleteMsg(dels)"
              >确认</el-button
            >
...
import { searchAllMessage, adminSendMsg, adminUpdateMsg, adminDeleteMsg } from '@/api/message'
...
export default {
  ...
  methods: {
    ...
    async openUpdate(row) {
      ...
      await setTimeout(() => {}, 100)
      this.$refs.edit.setHtml(row.content) //公告内容
    },
    async submitForm() {
      let data
