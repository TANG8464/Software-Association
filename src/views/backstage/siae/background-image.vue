  ...
  <div class="background-image">
    <div class="demo-image__placeholder">
      ...
      <div class="pictures" ref="pictures">
        ...
        >
          <div slot="actions" slot-scope="scope">
            <div class="actions">
              <span
                style="margin: 5%"
                v-show="!scope.item.status"
                @click="updateStatus(scope.item.id, true)"
              >
              ...
              <span
                style="margin: 5%"
                v-show="scope.item.status"
                @click="updateStatus(scope.item.id, false)"
              >
        ...
        <div v-else-if="isNotData"><icon name="null" scale="40" width="40"></icon>一张都没有</div>
      </div>
...
export default {
  ...
  watch: {
    size(newVal) {
      if (newVal.isSmallSize || newVal.isMediumSize) {
        this.col = 4
      } else this.col = 5
    },
  },
  ...
  methods: {
    ...
    async updateStatus(id, status) {
      ...
      if (data.code === 200) {
        this.setAllBackground(1, this.currPage * this.limit, true)
        this.$message.success(status == 1 ? '已设为可见' : '已设为不可见')
      } else this.$message.error(data.message)
    },
    //删除背景图
