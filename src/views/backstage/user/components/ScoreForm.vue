<template>
  ...
  >
    <el-form-item
      label="ID"
      prop="memberID"
      :rules="[{ required: true, message: '成员号不能为空' }]"
    >
      <select-user :selectId.sync="form.memberID"></select-user>
    </el-form-item>
    <el-form-item
      label="考核名称"
      prop="testName"
    ...
    >
      <el-input
        @change="$emit('update:scoreForm', form)"
        type="text"
...
export default {
  ...
  watch: {
    ...
    'form.memberID'() {
      this.$emit('update:scoreForm', this.form)
    },
  },
  mounted() {
