<template>
  <div class="bulk-import">
    ...
    <el-upload
      ref="upload"
      :class="{ 'upload-demo': !isSuccess, 'upload-btn': isSuccess }"
      :style="{ width: isSuccess ? '' : '100%', 'max-width': isSuccess ? '' : '500px' }"
      :drag="!isSuccess"
      :action="uploadUrl + 'member/uploadMemberExcel'"
      :headers="token"
...
import { excelVerify } from '@/utils/verify'
import { getBaseURL } from '@/utils/url'
import { downloadMemExportTemplates } from '@/api/member'
export default {
  components: {
    UploadTable,
  },
  ...
  methods: {
    async setUploadLink() {
      const res = await downloadMemExportTemplates()
      this.uploadLink = window.URL.createObjectURL(res)
    },
    beforeUpload(file) {
