<template>
<el-dialog :title="dialogTitle" :visible.sync="isOpen" v-dialogDrag :width="isSmallSreen?'100%':'30%'" @close="cancel()">
    <el-form :model="menuObj" v-if="menuObj" label-width="80px">
        ...
        <el-form-item label="父级菜单" style="text-aligin:left;" v-if="menuObj.isP">
            <el-select v-model="menuObj.parentId" placeholder="请选择" style="width:100%">
                <el-option v-for="item in parentArr" :key="item.menuId" :label="item.name" :value="item.menuId"></el-option>
            </el-select>
        </el-form-item>
        <el-form-item label="路由" v-if="btnType!==2">
            <el-input v-model="menuObj.url" autocomplete="off" placeholder="路由地址不包括/Backstage(设置为-则不展示)"></el-input>
        </el-form-item>
        ...
        <el-form-item label="图标" v-if="btnType!=2">
            <el-input type="textarea" v-model="menuObj.icon" autocomplete="off" maxlength="3000" show-word-limit resize="none" rows="8" placeholder="贴入表示图标的代码(建议宽高18~30px)"></el-input>
        </el-form-item>
