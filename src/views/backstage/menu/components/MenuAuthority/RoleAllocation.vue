...
<script>
import { getAllMenuNavTree, updateMenuNavStatus } from '@/api/menu'
import { permissionAssignment } from '@/api/menu/role'
export default {
  data() {
    return {
      menuTree: [], //权限菜单
  ...
  methods: {
    //设置所有权限
    async setMenuTree() {
      const res = await getAllMenuNavTree()
      if (res.code === 200) {
    ...
    getChecked(arr) {
      if (arr.menuNodeList && arr.menuNodeList.length != 0) {
        arr.menuNodeList.forEach((item) => {
          ...
          } else this.power.checked.push(item.menuId)
        })
      }
    },
    //分配权限
    async Distribution() {
      ...
      const res = await permissionAssignment(this.power.roleId, checked.concat(halfChecked))
      if (res.code === 200) {
