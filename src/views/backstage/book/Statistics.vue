...
<div ref="statistics" class="book-statistics">
    <div class="date">
        <el-date-picker v-model="date" type="daterange" align="right" unlink-panels range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" :picker-options="pickerOptions" value-format="timestamp" :default-time="['00:00:00', '23:59:59']"></el-date-picker>
        <span :style="{'display':size.isSmallSize?'block':'inline','text-align':'right'}" style="width:100%">
            <el-button type="primary" size="medium" icon="el-icon-search" class="magrin" @click="query()">查询</el-button>
        </span>
    </div>
    <div class="table">
        <el-table :data="bookData" style="width: 100%" :default-sort="{prop: 'bdate', order: 'descending'}" max-height="300" min-height="300">
        ...
        <el-pagination @current-change="handleCurrentChange" :current-page="pagenum" :page-size="10" layout="total, prev, pager, next, jumper" :total="total"></el-pagination>
    </div>
    <p id="title">借阅统计图</p>
...
export default {
    ...
    methods: {
        async handleCurrentChange(val) {
            const {
                data: res
            } = await this.$http.get('bookborrow/search', {
                headers: this.token,
                params: {
                    curPage: val
                },
        ...
        async getChartData(end, start) {
            ...
            const {
                data: res
            } = await this.$http.get('bookborrow/loanCount', {
                headers: this.token,
                params: {
                    endDay: end,
                    startDay: start
