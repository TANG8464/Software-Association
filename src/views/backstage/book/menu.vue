    ...
    <div class="menu">
        <div class="book-menu_search">
            ...
            <span :style="{ display: size.isSmallSize ? 'block' : 'inline', 'text-align': 'right' }" style="width: 100%">
                <el-button type="primary" size="medium" icon="el-icon-search" class="magrin search_btn" style="display: inline" @click="query()">查询</el-button>
            </span>
...
export default {
    ...
    methods: {
        ...
        async confirm(form) {
            ...
            const {
                data: res
            } = await this.$http.get('book/search', {
                params: this.form,
            })
            if (res.code !== 200) {
        ...
        async confirmBorrow(fromName) {
            ...
            const {
                data: res
            } = await this.$http.post('bookborrow/borrow', resData, {
                headers: this.token,
            })
            if (res.code != 200) {
                this.$message.error(res.message)
