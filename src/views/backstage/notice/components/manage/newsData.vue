...
<script>
import paging from "@/views/backstage/notice/components/manage/paging.vue";
export default {
  data() {
    return {
      noticeData: [], //所有公告
  ...
  filters: {
    ...
    timeFormat(val, flag) {
      if (flag === 2) {
        ...
        return `预计${date.getFullYear()}-${date.getMonth() < 9 ? '0' + (date.getMonth() + 1) : date.getMonth() + 1}-${date.getDate()} ${date.getHours()}:${date.getMinutes()}发送`;
      } else return val;
    }
  },
  components: {
