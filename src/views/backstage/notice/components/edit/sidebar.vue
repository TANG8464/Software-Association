    ...
    <el-col :span="24">
      <div class="block">
        ...
        <div v-if="operation.isTiming">
          <p class="small-title">发布时间</p>
          <datetime @setDate="setDate" ref="datetime"></datetime>
        </div>
      </div>
    </el-col>
    <el-col :span="24">
...
<script>
import datetime from '@/views/backstage/notice/components/edit/sidebar/datetime.vue';
export default {
  components: {
    datetime
  },
  ...
  methods: {
    setDate(val) {
      ...
      console.log(this.operation.date);
    },
    //设置top值
    setTop(top, highTop) {
      if (top && highTop) {
    ...
    handleInputConfirm() {
      ...
      if (label.length >= 6) {
        ...
        if (newLabel.includes(',') || newLabel.includes('，')) {
          this.$message.error(`出错啦，标签不能包含逗号`);
