...
<div class="allRes_table">
    <el-table ref="singleTable" :data="allRes" style="width: 100%;text-aligin:center;min-height:500px;" @row-contextmenu="more" @row-dblclick="more" v-loading="loading" element-loading-text="拼命加载中..." element-loading-background="rgba(255, 255, 255, 0.5)">
        <el-table-column label="文件" show-overflow-tooltip style="display: flex;align-items: center;">
            <template slot-scope="scope">
                <img class="resImg" v-if="scope.row.resourceLogo" :src="scope.row.resourceLogo" alt width="40" height="40" />
                <span v-text="scope.row.resourceName" style="margin: 0 5px;"></span>
            </template>
        </el-table-column>
        <el-table-column v-if="!size.isSmallSize" property="resourceCategory.resourceCategoryName" label="资源分类" width="100"></el-table-column>

        <el-table-column v-if="!size.isSmallSize" property="uploadDate" label="上传日期" width="200"></el-table-column>
        <el-table-column v-if="!size.isSmallSize" property="member.memberName" label="上传者" width="150"></el-table-column>
    ...
    <update-res :update.sync="update" :isOpenUpdate.sync="isOpenUpdate" :fileType="fileType" @setLogo="setLogo" @updateRes="updateRes"></update-res>
    <!--右击弹出更多设置-->
...
export default {
    ...
    methods: {
        ...
        async openDetails() {
            ...
            const {
                data
            } = await this.$axios.get(`/resources/${this.selected.id}`) //获取详细信息
            if (data.code === 200) {
        ...
        async updateStatus(row) {
            ...
            const {
                data
            } = await this.$axios.put(`resources/${row.id}/status?status=${row.status}`)
            if (data.code === 200) {
        ...
        async updateRes() {
            const {
                data
            } = await this.$axios.put(`resources/${this.selected.id}`, this.update)
            if (data.code === 200) {
        ...
        async delRes() {
            ...
            const {
                data
            } = await this.$axios.delete('resources/', {
                headers: {
