  ...
  <div class="resUpload-upload_information">
    ...
    <div style="margin: 10px">
      ...
      <el-popover placement="top" width="160" v-model="isOpenClearTip">
        <p><i class="el-icon-info" style="color: red"></i> 取消后将不会保留您输入的数据</p>
...
import { saveResourceInfo, cancelUpload } from '@/api/resource'
...
export default {
  ...
  methods: {
    ...
    async setResSort() {
      ...
      if (res.code === 200) {
        this.resourceCategory = res.data.records
    ...
    async uploadResInfo() {
      ...
      if (this.author === '1') {
        ...
        if (!this.$cookies.get('activeUser')) {
          //如果cookies被恶意清除了则加上
          const res = await getActiveUserInfo()
          this.$cookies.set('activeUser', res.data.id + '&' + res.data.memberName)
        }
        this.res.author = this.$cookies.get('activeUser').split('&')[1]
      }
      const res = await saveResourceInfo(this.res)
      if (res.code === 200) {
