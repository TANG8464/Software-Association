    ...
    <div v-if="!uploadProgress.isUpload">
      ...
      >
        ...
        <div class="el-upload__tip" slot="tip">
          为节省服务器资源请尽量上传容量小的文件，最大不可超过5GB（尽量选择本地服务器，七牛云作为备用存储）
        </div>
      </el-upload>
    </div>
        ...
        <el-col :xs="18" :lg="21">
          <p>
            ...
            <small
              class="progress_tip"
              v-text="
                fileSizeFormatter(uploadProgress.loaded) +
                '/' +
                fileSizeFormatter(uploadProgress.fsize)
              "
            ></small>
            <small
              class="progress_tip"
              v-if="uploadProgress.percentage != 100"
              v-text="fileSizeFormatter(uploadProgress.nSpeed)"
            ></small>
...
import { getDownloadToken } from '@/api/resource'
export default {
  data() {
    return {
      //存储位置
  ...
  methods: {
    dataInit() {
      ...
      this.serverLink = ['', `${getBaseURL()}resources/upload/local`, 'http://upload-z2.qiniup.com']
      //页面载入前获取七牛云上传凭证
      this.getQiniuToken()
    },
    async getQiniuToken() {
      ...
      //生成临时文件名
      const fileName = `${nowDate.getFullYear()}${month}${nowDate.getDate()}${nowDate.getHours()}${nowDate.getMinutes()}${nowDate.getSeconds()}`
      ...
      const res = await getDownloadToken(fileName)
      this.data['2'].token = res.data
    },
    fileSizeFormatter,
    beforeAvatarUpload: function (file) {
      const filenameArr = file.name.split('.')
      const fileType = filenameArr[filenameArr.length - 1] //文件类型
      this.$emit('update:fileType', fileType) //将文件类型传给父组件,用作绘制封面
      const size = 1024 * 1024 * 1024 * 5 //1GB
      if (file.size < size) {
