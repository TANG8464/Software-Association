    ...
    >
      <li class="sortArr" v-for="(item,index) in allSort" :key="item.id" :data-index="index">
        <el-row :gutter="24" style="width:100%;text-align:center;">
          ...
          <el-col :span="7">
            <h4 style="margin:16px 0">{{item.resourceCategoryName}}</h4>
          </el-col>
    ...
    >
      <el-form :model="updateSort">
        ...
        <el-form-item label="分类名称">
          <el-input v-model="updateSort.resourceCategoryName" autocomplete="off"></el-input>
        </el-form-item>
...
<script>
import updateIcon from "@/views/backstage/resource/components/sort/sortData/updateIcon.vue";
import delIcon from "@/views/backstage/resource/components/sort/sortData/delIcon.vue";
export default {
  data() {
    return {
      allSort: [], //所有分类
  ...
  methods: {
    ...
    async delSort(ids) {
      try {
        let res = await this.$axios.delete("resocategory/", {
          data: ids
        });
        let type = "";
    ...
    async update(id) {
      try {
        let res = await this.$axios.put("resocategory/", this.updateSort);
        if (res.status === 200) {
          let type = "";
          if (res.data.code === 200) type = "success";
          else type = "error";
