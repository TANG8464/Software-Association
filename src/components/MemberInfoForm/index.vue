<template>
<el-form label-position="top" label-width="80px" status-icon :rules="rules" v-if="info" :model="info" ref="info" class="member-info-form">
    <el-form-item v-if="show.includes('memberName')" label="名称" prop="memberName" :style="{ 'max-width': `${maxWidth('memberName')}px` }">
        <el-input v-model="info.memberName"></el-input>
    </el-form-item>
    <el-form-item v-if="show.includes('gender')" label="性别" prop="gender" :style="{ 'max-width': `${maxWidth('gender')}px` }">
        <el-radio v-model="info.gender" label="男">男</el-radio>
        <el-radio v-model="info.gender" label="女">女</el-radio>
    </el-form-item>
    <el-form-item v-if="show.includes('startYear')" label="届数" prop="startYear" :style="{ 'max-width': `${maxWidth('startYear')}px` }">
    ...
    <el-form-item v-if="show.includes('cascadeClass')" label="班级" :style="{ 'max-width': `${maxWidth('cascadeClass')}px` }">
        <el-cascader v-model="info.cascadeClass" :options="cascadeClass" :props="{ expandTrigger: 'hover' }" @change="handleChangeCascadeClass"></el-cascader>
    </el-form-item>
    <el-form-item v-if="show.includes('stu_num')" label="学号" prop="stu_num" :style="{ 'max-width': `${maxWidth('stu_num')}px` }">
        <el-input v-model="info.stu_num"></el-input>
    </el-form-item>

    <el-form-item v-if="show.includes('nation')" label="国籍" prop="nation" :style="{ 'max-width': `${maxWidth('nation')}px` }">
        <el-input v-model="info.nation" value="中国"></el-input>
    </el-form-item>
    <el-form-item v-if="show.includes('nativePlace')" label="家乡" prop="nativePlace" :style="{ 'max-width': `${maxWidth('nativePlace')}px` }">
        <provincial-linkage :place.sync="info.nativePlace"></provincial-linkage>
    </el-form-item>
    <el-form-item v-if="show.includes('address')" label="所在地" prop="address" :style="{ 'max-width': `${maxWidth('address')}px` }">
        <provincial-linkage :place.sync="info.address"></provincial-linkage>
    </el-form-item>
    <el-form-item v-if="show.includes('education')" label="学历" prop="education" :style="{ 'max-width': `${maxWidth('education')}px` }">
    ...
    <el-form-item v-if="show.includes('politicalOutlook')" label="政治面貌" prop="politicalOutlook" :style="{ 'max-width': `${maxWidth('politicalOutlook')}px` }">
        <el-select placeholder="请选择" v-model="info.politicalOutlook">
            <el-option v-for="item in politicalOutlooks" :key="item.value" :label="item.label" :value="item.value"></el-option>
        </el-select>
    </el-form-item>
    <el-form-item v-if="show.includes('cardID')" label="身份证" prop="cardID" :style="{ 'max-width': `${maxWidth('cardID')}px` }">
        <el-input v-model="info.cardID"></el-input>
    </el-form-item>
    <el-form-item v-if="show.includes('phoneNum')" label="手机号" prop="phoneNum" :style="{ 'max-width': `${maxWidth('phoneNum')}px` }">
        <el-input v-model="info.phoneNum"></el-input>
    </el-form-item>
    <el-form-item v-if="show.includes('qq')" label="QQ" prop="QQ" :style="{ 'max-width': `${maxWidth('qq')}px` }">
        <el-input v-model="info.qq"></el-input>
    </el-form-item>
    <el-form-item v-if="show.includes('email')" label="邮箱" prop="email" :style="{ 'max-width': `${maxWidth('email')}px` }">
        <el-input v-model="info.email"></el-input>
    </el-form-item>
    <el-form-item v-if="show.includes('description')" label="个人说明" prop="description" :style="{ 'max-width': `${maxWidth('description')}px` }">
...
export default {
    ...
    methods: {
        initStartYear() {
            ...
            this.info.startYear = currentYear
        },
        maxWidth(value) {
        ...
        resetForm() {
            ...
            this.info = info
            this.info.cascadeClass = this.info.iclass ? [this.info.institute.id, this.info.specialty.id, this.info.iclass.id] : []
        },
    },
}
</script>
