            ...
            <div class="msg-item" v-for="(item,index) in msg" :key="item.id" @click="onselect(item,index)">
                ...
                <div class="msg-main">
                    ...
                    <p class="msg-footer">
                        ...
                        <span>{{item.createTime|dataFormatter}}</span>
                    </p>
                </div>
                <span class="actions">
...
export default {
        ...
        isChecked(newVal) {
            ...
            if (newVal) {
                el.children.forEach((item, index) => {
                    if (!item.getAttribute('data-checked')) item.setAttribute('data-checked', true)
                })
                this.setSelectItem()
            } else {
    ...
    methods: {
        ...
        setSelectItem() {
            ...
            const el = this.$refs['msg-list']
            el.children.forEach((item, index) => {
                if (item.getAttribute('data-checked')) this.selectItem.push(this.msg[index].id)
            })
        },
    },
    filters: {
