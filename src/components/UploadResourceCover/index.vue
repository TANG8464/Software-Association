...
<div class="upload-information_cover">
    ...
    <el-row :gutter="24">
        <el-col :xs="10" :lg="8">
            <el-upload class="avatar-uploader" action="http://120.26.177.203/news/picturesUpload" :show-file-list="false" :on-success="handleAvatarSuccess" :on-error="handleError" :before-upload="beforeAvatarUpload" :on-progress="handleProgress" :headers="token" :style="{'width':`${imgWidth}px`,'height':`${imgWidth}px`}">
                <img v-if="imgUrl" :src="imgUrl" class="avatar" :style="{'width':`${imgWidth}px`,'height':`${imgWidth}px`}" />
                <i v-if="!imgUrl&&!isUpload" class="el-icon-plus avatar-uploader-icon" :style="{'width':`${imgWidth}px`,'line-height':`${imgWidth}px`}"></i>
                <el-progress :width="size.isSmallSize?80:150" :stroke-width="3" v-if="!imgUrl&&isUpload" type="circle" :percentage="uploadProgress.percentage" :status="uploadProgress.status"></el-progress>
            </el-upload>
        </el-col>
        <el-col :span="12">
            <!--可选封面-->
            <p class="choosable-title">可选</p>
            <img :src="imgDataUrl" class="choosable-img" :style="{'width':`${imgWidth-25}px`,'height':`${imgWidth-25}px`}" @click="uploadCover()" />
        </el-col>
    </el-row>
</div>
</template>
    ...
    methods: {
        setCover() {
            ...
            img.onload = () => {
                ...
                if (this.fileType) ct.fillText(this.fileType.toUpperCase(), 42, 96)
